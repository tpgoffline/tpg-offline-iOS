...
class RoutesListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if loading == true {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "listeItineaireCell", for: indexPath) as! RoutesListTableViewCell
            if ContrastColorOf(AppValues.primaryColor, returnFlat: true) == FlatWhite() {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "voirItineraire" {
            let destinationViewController: RouteDetailTableViewController = (segue.destination) as! RouteDetailTableViewController
