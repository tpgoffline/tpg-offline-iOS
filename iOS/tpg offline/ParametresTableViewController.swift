...
class ParametresTableViewController: UITableViewController {
	...
	func afficherTutoriel() {
		...
		iconeF.addAttribute(NSForegroundColorAttributeName, value: AppValues.textColor)
		let page5 = OnboardingContentViewController (title: "Itinéraires".localized(), body: "l’application propose un menu Itinéraires (majuscule à Itinéraires, et au pluriel, tu fais référence à un titre). Vous pouvez vous déplacer très facilement grâce à cette fonction.".localized(), image: iconeF.imageWithSize(CGSize(width: 50, height: 50)), buttonText: "Continuer".localized(), actionBlock: nil)
