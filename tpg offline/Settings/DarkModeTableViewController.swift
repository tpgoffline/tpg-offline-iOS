...
class DarkModeTableViewController: UITableViewController {
  ...
  override func tableView(_ tableView: UITableView,
    ...
    let cell =
      tableView.dequeueReusableCell(withIdentifier: "darkModeRow", for: indexPath)
    ...
    if indexPath.section == 0 {
      ...
      cell.detailTextLabel?.text = Text.fromSunsetSunrise
      cell.detailTextLabel?.textColor = App.textColor
      statusSwitch.isOn = App.automaticDarkMode
      statusSwitch.addTarget(self, action: #selector(self.changeAutomatic),
                             for: .valueChanged)
    } else {
      cell.textLabel?.text = Text.manual
      cell.textLabel?.textColor = App.textColor
      cell.detailTextLabel?.text = ""
      statusSwitch.isOn = App.darkMode
      statusSwitch.addTarget(self, action: #selector(self.changeManual),
                             for: .valueChanged)
    }
