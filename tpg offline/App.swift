...
struct App {
  #if os(iOS)
  static var watchSessionManager = WatchSessionManager.shared
  #endif
  ...
  static var darkMode: Bool {
    get {
      return false //return (UserDefaults.standard.bool(forKey: #function))
    }
    set {
      UserDefaults.standard.set(newValue, forKey: #function)
      #if os(iOS)
  ...
  @discardableResult static func loadStops(forceLocal: Bool = false) -> Bool {
    do {
      ...
      for (i, id) in FavoritesManager.shared.stops.enumerated() {
        if App.stops.filter({ $0.appId == id })[safe: 0] == nil {
          FavoritesManager.shared.stops.remove(at: i)
        }
      }
      return true
    } catch {
  ...
  static func imageForStop(stop: Stop) -> UIImage? {
    guard let dirString = NSSearchPathForDirectoriesInDomains(.documentDirectory,
                                                              .allDomainsMask,
                                                              true).first else {
                                                                return nil
    }
    let path = URL(fileURLWithPath: dirString).appendingPathComponent("image\(stop.appId).jpeg")
    do {
